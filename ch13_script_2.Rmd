...
tree_mod <- decision_tree(cost_complexity = 0.01) %>%
  set_engine("rpart", 
             model = TRUE) %>%
...
nes %>%
filter(ideology < 4) %>%
  summarise(mean = mean(dvote))

ggplot(nes, aes(ideology, dvote)) + geom_point() + geom_jitter()
...
```{r s3, echo=FALSE}
forest_mod <- rand_forest(trees = 500) %>%
